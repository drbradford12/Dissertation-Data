...
<!-- Sanders incorporates contour binning into X-ray spectroscopy in scientific imaging to ensure the preservation of the relative order of data, particularly when dealing with spatially resolved data, emphasizing the significance of maintaining data hierarchy for accurate analysis. Contour binning is particularly useful when dealing with variations in spatial density as it arranges data points based on their spatial movement and organization. It keeps the relative distance and order very well by sorting pixels by their flux before binning. This makes it perfect for image-based scientific research, where continuity and order in space are fundamental [@sanders2006].

Diehl and Statler propose adaptable binning for X-ray data, advocating for dynamically adjusting bin sizes according to data density to optimize data representation and preserve relative ordering, ensuring efficient analysis of X-ray datasets. This flexible method works well in areas with a lot of data because it uses smaller bins to keep the relative order and detail. Larger bins can be used without significant data loss in scenarios with limited data. Astronomical data analysis uses this method, which works nicely for datasets with significantly different densities. It keeps more detailed ordering where needed and lets bigger bins fit in places with few data points [@diehl2006].

Wu et al. propose FastBit, a rapid binning technique ideal for querying extensive datasets efficiently, as it enables quick data retrieval without compromising the original data order, enhancing speed and accuracy in data analysis. FastBit lets the user quickly query big datasets without messing up the order of the data points by rounding numbers to a single significant digit and using ad hoc binnining strategies. This technique is highly effective for handling extensive datasets where speed and accuracy are important. It strikes a good mix between reducing the amount of data and accurately showing the original numerical order [@wu2009]. -->

Hauskrecht and Xue examine how to train classification models and the importance of bins for keeping data in order while sorting values. They use equal-distance binning to divide number ranges without messing up the order too much. This method effectively sorts things into groups while keeping the basic order. It does this without losing the interpretive value of number distances, which is important for tasks that depend on how close two data points are to each other [@xue2017].
