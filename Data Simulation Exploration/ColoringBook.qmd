---
title: "Simulating the bins: Determine the distribution layouts"
editor: source
format: 
  pdf:
...
Each section contains a histogram grid displaying the impact of different bin sizes (0.5, 1, 2, 5, 10) on the distribution's appearance.

- Small Binwidths: Highlight fine-grained details, preserve discreteness (where applicable), and maintain visible gaps in the data.

- Large Binwidths: Smooth the histogram, reduce detail, merge gaps, and provide a more generalized view of the data distribution.
...
binwidths <- c(0.5, 1, 2, 5, 10)

point_mass_data <- simulate_point_mass(n, 5)
...
exponential_data <- simulate_exponential(n, rate = 0.5)
...
normal_data <- simulate_normal(n, mean = 10, sd = 2)
