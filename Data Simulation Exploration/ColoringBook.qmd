...
# Solution #1
numerical_tie_breaker_solution1 <- function(values, tie_band = 0.1) {
  # Sort the values
...
numerical_tie_breaker_solution2 <- function(values, base_fraction = 0.1) {
...
create_simulated_data_test <- function(data, data_type, tie_band_value = 0.01, dodge_value = 0.01, scale_factor = 0.1){
...
df_pcp <- df %>%
  pcp_select(original_dec1, manual_tie_band, group, original_dec1, auto_tie_band) %>%
  pcp_scale(method = "uniminmax") %>%
  pcp_arrange(method = "from-left")

# Create the parallel coordinate plot using ggpcp
return(
  ggplot(df_pcp, aes_pcp()) +
  ...
  geom_point(aes(colour = group), position = position_dodge(dodge_value)) +
  labs(
    title = paste("Parallel Coordinate Plot: Original vs Adjusted Data (Simulated", data_type,  "Data)", sep = ' '),
    x = "Variables",
    y = "Scaled Values"
...
create_simulated_data_test(exponential_data, "Exponnential")
