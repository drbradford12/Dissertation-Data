...
img_van_der <- here::here("comprehensive_exam", "images", "Van_der_corput_distribution.jpeg")
...
\footnotesize
**Key Frustration & Insight:** Without tie resolution, the dashboard was unusable. 
The massive overplotting from numerical ties (e.g., 30 towns with the exact same median income) meant individual community profiles were completely obscured. This visual occlusion prevented data-driven decision-making, as users could not distinguish, select, or compare their town against other similar communities—the dashboard's primary goal.
...
<!-- ## Implications for Visualization Science -->

<!-- **Methodological:** -->

<!-- - Biomimetic approaches validated for visualization -->
<!-- - Determinism as design requirement established -->
<!-- - Trade-off analysis framework demonstrated -->
...
- **Adding:** Numerical ties via deterministic jittering
...
## Core Concept: Jittering

**Definition:** Controlled displacement preventing visual overlap while preserving approximate position
...
|--------|---------------|-----------|
| **Magnitude** | Parameter $\epsilon$ (0.05-0.10 of range) | Balance visibility/validity |
| **Purpose** | Visual separation only | Not data modification |
| **Constraint** | Minimal displacement | Maintain perceptual truth |
| **Requirement** | Uniform distribution | Faithful density representation |

\normalsize

## Core Concept: Determinism

**Deterministic Methods:** Identical input $\rightarrow$ Identical output (always)
...
| Scientific validity | Verifiable | Unreliable |
| Artifacts | None | Clustering/gaps |
| Examples | Halton, Sunflower | Random uniform |
| **Our choice** | **Required** | **Rejected** |

\normalsize

\vspace{0.2cm}

**Implication:** Determinism essential for reproducible science [@peng2011]

## Evaluation Framework: "Best For"

**Interpretation:** "Best for" = optimal balance of competing demands in specific context
...
|-----------|----------------|
| **1. Task requirements** | Density estimation, clustering, outlier detection |
| **2. Data characteristics** | Tie group sizes, dimensions, distributions |
| **3. Performance criteria** | Accuracy, speed, perceptual quality, cognitive load |
| **4. Trade-off optimization** | Method strengths aligned with scenario needs |
...
3. **Our contribution**: Uniformity adjustmeny
...
- **H2**: Halton < Random < No Jitter (time)
