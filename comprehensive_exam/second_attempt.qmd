...
Let $\mathcal{D}$ be a dataset consisting of $n$ observations and $p$ variables. 
We define:

* $\boldsymbol{X} = \{X_1, X_2, \ldots, X_p\}$: the ordered set of variables displayed on parallel axes.
* $x_{ij}$: the value of observation $i$ on variable $j$, where $i \in \{1, \ldots, n\}$ and $j \in \{1, \ldots, p\}$.
* $y_{ij}$: the scaled y-coordinate of observation $i$ on axis $j$, normalized to $[0, 1]$.
* $\tilde{y}_{ij}$: the adjusted y-coordinate after tie-breaking transformation.

#### Scaling Transformation

For numerical variables, the standard min-max normalization (termed "uniminmax" scaling) transforms raw values to the unit interval:

$$y_{ij} = \frac{x_{ij} - \min(X_j)}{\max(X_j) - \min(X_j)}$$ {#eq-scaling}

#### Tie Definition

::: {#def-tieset}
##### Tie Set
For a given variable $X_j$, we define the **tie set** at value $v$ as:
$$T_j(v) = \left\{ i : |y_{ij} - v| < \varepsilon \right\}$$ {#eq-tie_set}
where $\varepsilon$ is a numerical tolerance (default $\varepsilon = 10^{-10}$).
:::

A value $v$ constitutes a *tie* if and only if $|T_j(v)| > 1$.

### Available Space Computation

#### Neighbor Identification

For a tied value $v$ on axis $j$, we identify the nearest distinct neighboring values. 
Let $\mathcal{Y}_j$ denote the set of all unique scaled values on axis $j$.

::: {#def-lower-neighbor}
##### Lower Neighbor
$$v^{-} = \max\left\{ y : y \in \mathcal{Y}_j \setminus \{v\},\, y < v \right\}$$ {#eq-lower_neighbor}
:::

::: {#def-upper-neighbor}
##### Upper Neighbor
$$v^{+} = \min\left\{ y : y \in \mathcal{Y}_j \setminus \{v\},\, y > v \right\}$$ {#eq-upper_neighbor}
:::

### Boundary Computation

The available region for spreading tied observations is bounded by midpoints to neighboring values:

$$L(v) = 
\begin{cases}
\frac{v + v^{-}}{2} & \text{if } v^{-} \text{ exists}\\
\max(0, v - \beta) & \text{otherwise}
\end{cases}$$ {#eq-lower_bound}

$$U(v) = 
\begin{cases}
\frac{v + v^{+}}{2} & \text{if } v^{+} \text{ exists}\\
\min(1, v + \beta) & \text{otherwise}
\end{cases}$$ {#eq-upper_bound}

where $\beta$ is the density bandwidth parameter (default $\beta = 0.05$).

### Local Density Adjustment

To prevent excessive spread in sparse regions, we compute local density using a kernel estimate:

$$\rho_j(v) = \frac{1}{n} \cdot |\{ i : |y_{ij} - v| < \beta \}|$$ {#eq-local_density}

The **density factor** scales the available space:

$$\varphi(v) = 1 - 0.5 \cdot \rho_j(v)$$ {#eq-density_factor}

### Final Available Space

The adjusted available space incorporates both boundary constraints and density adjustment:

$$S(v) = \max\left\{ (U(v) - L(v)) \cdot \varphi(v),\; \delta \cdot (|T_j(v)| - 1) \right\}$$ {#eq-available_space}

where $\delta$ is the minimum spacing parameter (default $\delta = 0.01$).

### Optimal Spacing Formula

The fundamental principle mirrors the `ggpcp` categorical approach: distribute observations evenly within the available space. For $k = |T_j(v)|$ tied observations, the **optimal inter-observation spacing** is:

$$\Delta = \frac{S(v)}{k - 1}$$ {#eq-optimal_spacing}


### Hierarchical Ordering for Crossing Minimization

#### Sorting Key Construction

For observation $i \in T_j(v)$, define the **sorting key** based on adjacent variables $\mathcal{A}_j = \{X_{a_1}, X_{a_2}, \ldots, X_{a_m}\}$:

$$\kappa_i = ( y_{i,a_1}, y_{i,a_2}, \ldots, y_{i,a_m} )$$ {#eq-sorting_key}

The tied observations are ordered lexicographically:

$$\pi = \text{argsort}\left( \{ \kappa_i : i \in T_j(v) \} \right)$$ {#eq-ordering}


#### Position Assignment

Positions are assigned symmetrically around the original tied value:

$$\tilde{y}_{i_m, j} = s + (m - 1) \cdot \Delta$$ {#eq-position_assignment}

where $s$ is the start position calculated from the center $c = \frac{L(v) + U(v)}{2}$.
...
#| eval: false
#| label: alg-unified
#| caption: "Unified Tie-Breaking: pcp_arrange_unified"

Algorithm: Unified Tie-Breaking
Input: pcp_data, method, sigma, epsilon, beta, delta

For each axis j from 1 to p:
    If x_j is categorical:
        Apply categorical tie-breaking box allocation

    Else if x_j is numerical:
        Identify tie sets T_j(v)
    
        For each tie set:
            Compute S(v) using density and neighbors
            Compute hierarchical order pi based on adjacent axes
            Assign new positions y_tilde using optimal spacing Delta
Return adjusted pcp_data
```
