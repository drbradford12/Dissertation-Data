...
![Comparison of parallel coordinate plots without (left) and with (right) numerical tie-breaking. When observations share the same numerical value, their lines overlap completely, hiding how many cases exist at that value. Spreading tied values within resolution-based boxes (blue) makes each observation visible while signaling that positions within the box are artifacts of tie-breaking. ](images/pcp_line_tracing_guide_images/pcp_tiebreaking_updated.png){#fig-figure-8}
...
To minimize line crossings, tied observations are sorted by their positions on adjacent axes. 
For axis $j$, let $A_j$ be the adjacent axis (left or right depending on processing direction).
...
4. For each axis j in processing order:
     ...
     b. For each $v \in V$:
          ...
          iii. Order $T_{i}(v)$ by positions on adjacent axis
