...
Begin by identifying each vertical axis in the plot. Each axis represents one variable from the dataset. 
...
![Values are read at each axis intersection point.](images/pcp_line_tracing_guide_images/figure-4.png){#fig-figure-4 width=90%}
...
The ggpcp package addresses this through the even_tie_spread algorithm.
...
## The Solution: Even Tie Spreading

The `even_tie_spread` algorithm separates tied observations by distributing them evenly within a small range centered on their original value. 
By default, this range spans 2% of the data range, sufficient to maintain visual separation while preserving the approximate position of values.

![Even tie spreading maintains visual separation while preserving approximate positions.](images/pcp_line_tracing_guide_images/figure-7.png){#fig-figure-7 width=90%}

## Hierarchical Sorting for Minimal Crossings

Beyond simply spreading tied values, the ggpcp package implements hierarchical sorting to minimize line crossings. 
Observations within a tie group are ordered based on their values on adjacent axes. 
This ordering leverages the Gestalt principle of common fate: observations with similar trajectories appear near each other, creating visual bands that facilitate perception of groups moving together through the high-dimensional space.
...
Colin Ware's seminal work "Information Visualization: Perception for Design" provides a comprehensive framework for understanding how humans process visual information [@ware2012, 3rd ed., pp. 20-21, Figure 1.11]. 
...
### Hypothesis 1: Categorical-Numerical Consistency

**H1**: Using consistent visual encoding (equispaced lines for both categorical and numerical variables) reduces cognitive load compared to mixed encoding strategies.
...
**Study 1: Frequency Perception (H1)**
...
### Phase 3: Comparative Benchmarking (Weeks 6-18)
...
### Phase 4: Integration and Dissemination (Weeks 8-9)
...
| 5-7 | Study 1: Frequency | Raw data + analysis | Magnitude estimation |
| 8-10 | Benchmarking | Performance report | Computational metrics |
| 10-12 | Integration | Draft manuscript | Synthesis |
...
- Study 1 can run partially in parallel (Weeks 8-10)
- Benchmarking overlaps with final studies (Weeks 10-12)
