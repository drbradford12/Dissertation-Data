...
$$ {#eq-space}
<!--
![Computing available space. The algorithm uses the global axis resolution to define a consistent jitter interval around each tied value, preserving the relative spacing between distinct values.](images/math_images/section7_compute_available_space.png){#fig-available-space width=90%}
-->
...
<!--
![The even tie spread algorithm. Original tied values (left) are spread evenly within the computed width (right), with width determined by the global minimum gap between unique values.](images/math_images/section3_even_tie_spread.png){#fig-even-spread width=90%}
-->
...
<!--
![Hierarchical ordering minimizes line crossings. By sorting tied observations based on their positions in adjacent axes, lines that would otherwise cross are kept parallel, supporting the Gestalt principle of "common fate."](images/math_images/section8_hierarchical_ordering.png){#fig-hierarchical width=90%}
-->
...
<!--
![Position assignment. Observations are distributed evenly within the available space, with positions calculated as offsets from the lower bound.](images/math_images/section9_assign_spread_positions.png){#fig-assign-positions width=90%}
-->
...
<!--
![Position functions for even spacing. `position_even()` excludes edges (observations never touch boundaries), while `position_even_edges()` includes edges (first and last observations at boundaries).](images/math_images/section2_position_functions.png){#fig-position-functions width=85%}
-->
...
<!--
![The unified framework processes both categorical and numerical variables with consistent logic. Categorical variables receive proportional box allocation; numerical ties receive density-based spacing. Both use the same core spacing equation.](images/math_images/section10_unified_framework.png){#fig-unified width=95%}
-->
...
![Parallel coordinate plot after applying uniform spacing. Gray boxes indicate regions where tied values have been spread, making individual observations traceable.](images/math_images/section4_uniform_spacing_pcp.png){#fig-uniform-spacing width=90%} 

#### Annotated Tie Regions

Tie regions can be annotated with boxes showing the original value and the spread region:

![Annotated tie regions with labels. Gray boxes highlight where ties occurred, with labels showing the original tied value. This is particularly useful for integer data where ties are frequent.](images/math_images/section6_annotated_ties.png){#fig-annotated width=90%}

### Tie Box Detection

The `get_tie_box()` function identifies and bounds tie regions for visualization:

![The tie box detection process. (1) Identify duplicated values, (2) scale and group contiguous spreads, (3) generate bounding box coordinates for annotation.](images/math_images/section5_get_tie_box.png){#fig-tie-box width=90%}
...
4. For each axis j in processing order:
     ...
     b. For each $v \in V$:
          ...
          ii.  Compute bounds: $L(v) = v -\frac{\delta_i}{2}2, U(v) = v + \frac{\delta_i}{2}$
...
Based on prior graphical perception studies—including Heer and Bostock's [-@heer2010] crowdsourced replication of Cleveland and McGill [-@cleveland1984], which used approximately 50 participants per condition, and Hofmann and Vendettuoli's [-@hofmann2013] common angle plot study with 46 participants, we anticipated a medium effect size (Cohen's d = 0.5). For a two-tailed independent samples t-test with α = 0.05 and power = 0.80, the required sample size is n = 64 per group (total N = 128). 
...
This proposal addresses a fundamental challenge in information visualization: grounding design decisions in perceptual science rather than convention or intuition. 
The proposed equispaced line method extends ggpcp's established categorical tie-breaking algorithm to numerical variables, creating a unified framework that preserves visual continuity and leverages preattentive processing through the Gestalt principles of good continuation and common fate.

It is important to distinguish between two different uses of box-like visual elements in parallel coordinate displays. 
...
The frame signals that positions within it reflect tie-breaking rather than meaningful data variation, but individual lines remain continuous and traceable. 
Using a consistent jitter range across all values ensures that the visual representation preserves both the density of observations at each value and the gaps between distinct values; an individualized approach would risk destroying this distance information.
