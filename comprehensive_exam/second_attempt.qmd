...
![Colin Ware's three-stage model of visual information processing, illustrating the flow from low-level feature extraction to goal-directed processing (adapted from Ware, 2012).](images/ware_three_stage_model.png){#fig-ware-stages width="90%"}
...
![Form-based pop-out: orientation and size.](mages/fig_preattentive_form.png){#fig-preattentive-form width="90%"}
...
![Color-based pop-out: hue; plus an intensity ramp.](images/fig_preattentive_color.png){#fig-preattentive-color width="90%"}
...
![Spatial position (x,y) and implied depth via shading.](images/fig_preattentive_position_depth.png){#fig-preattentive-position width="90%"}
...
![Lace Padilla's cognitive model (Padilla et al., 2018).](images/padilla_processing.png){#fig-padilla width="90%"}
...
However, for frequency estimation tasks, constant-width boxes may have an advantage. Ware notes that area is preattentively processed, allowing immediate perception of relative quantities. 
...
where: - $start_i = \sum_{k=1}^{i-1}(S_k + space)$ is the starting position for value $v_i$ - $buffer$ provides margin space at boundaries (typically 0.02 × $S_i$) - The formula ensures even distribution across available space
